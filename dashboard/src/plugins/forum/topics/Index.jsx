          ...
          <Table bordered={true} rowKey="id" dataSource={this.state.items} columns={[
              {
                ...
                key: 'title',
                render: (text, record) => (<a href={`/forum/htdocs/articles/${record.id}`} target="_blank">{record.title}</a>)
              }, {
                title: 'Action',
                key: 'action',
                render: (text, record) => (<span>
                  <CopyToClipboard text={`/forum/htdocs/articles/${record.id}`}><Button shape="circle" icon="copy"/></CopyToClipboard>
